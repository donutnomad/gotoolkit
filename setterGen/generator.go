...
func generateSetterFileForMultiple(filename string, structInfos []*StructInfo, enableClean bool) error {
	if len(structInfos) == 0 {
		return fmt.Errorf("没有结构体需要生成")
	}

	// 使用第一个结构体的包名
	packageName := structInfos[0].PackageName
...
// safeParamName 生成安全的参数名（避免Go关键词）
