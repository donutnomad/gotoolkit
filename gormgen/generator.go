...
func generateGormQueryFileForMultiple(filename string, models []*gormparse.GormModelInfo) error {
	...
	builder.WriteString(fmt.Sprintf("// Code generated by %s. DO NOT EDIT.\n", name))
...
// generateGormQueryAndPatchFile 生成包含query和patch的合并文件
func generateGormQueryAndPatchFile(filename string, models []*gormparse.GormModelInfo) error {
	if len(models) == 0 {
		return fmt.Errorf("[gormgen] 没有模型需要生成")
	}
	...
	// query需要的导入
	allImports["github.com/donutnomad/gotoolkit/lib/gsql/field"] = true
	allImports["github.com/donutnomad/gotoolkit/lib/gsql"] = true

	// patch需要的导入
	for _, model := range models {
		imports := getGormRequiredImports(model)
	...
	slices.Sort(thirdPartyImports)
	for _, imp := range thirdPartyImports {
		if imp == "github.com/donutnomad/gotoolkit/lib/gsql" {
			builder.WriteString("\tgsql \"github.com/donutnomad/gotoolkit/lib/gsql\"\n")
		} else {
	...
	for i, model := range models {
		...
		// 生成Patch结构体
		builder.WriteString(generateGormPatchStruct(model))
		builder.WriteString("\n\n")

		// 生成Build方法
		builder.WriteString(generateBuildMethod(model))
		builder.WriteString("\n\n")

		// 生成Add方法
		builder.WriteString(generateAddMethod(model))
		builder.WriteString("\n\n")

		// 生成setter方法
		builder.WriteString(generateFieldSetterMethods(model))
	}
