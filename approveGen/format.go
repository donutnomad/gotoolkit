...
func formatOut1(fileName string, src []byte) ([]byte, error) {
	fileSet := token.NewFileSet()
	// Try as whole source file.
	file, err := parser.ParseFile(fileSet, fileName, src, parser.AllErrors|parser.ParseComments)
	if err != nil {
		panic(err)
	}
	// 格式化
	return formatOut(fileSet, file)
}

func formatOut(fset *token.FileSet, file *ast.File) ([]byte, error) {
	var LocalPrefix = ""
	var TabIndent = true
	var TabWidth = 8
	//var adjust func(orig []byte, src []byte) []byte = nil
	...
	//}
	mergeImports(file)
	sortImports(LocalPrefix, fset.File(file.FileStart), file)
	...
	printConfig := &printer.Config{Mode: printerMode, Tabwidth: TabWidth}
